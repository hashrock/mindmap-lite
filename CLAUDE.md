...
- **マルチルートはノートごとに無効化できる**（`MindMapModel.multiRoot === false`）。エディタ右上の切り替え（`MultiRootToggle` → `setMultiRoot`）でノートに保存される（`content` JSON の一部。専用の DB カラムは無い — 文書の一部という以上の意味を持たないので `notes` テーブルとは別に持たせない）。無効時は `addRootAt` 自身が「既に木が1つあれば no-op」を強制する — UI側でメニュー項目を隠すのは案内であって、その隠蔽が不変条件そのものではない（隠し忘れても `addRootAt` が壊れない）。既存の複数木を遡って1つにまとめることはしない（オフにした後も既存の木はそのまま）。
